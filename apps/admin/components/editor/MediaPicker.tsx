...
export function MediaPicker({ onSelect, onClose, multiple = false, onSelectMany }: Props) {
  ...
  return <AdminModal title="Select media" onClose={onClose}><div className="picker-toolbar"><input aria-label="Search media" placeholder="Search images" value={search} onChange={e => setSearch(e.target.value)}/><select aria-label="Collection" value={collection} onChange={e => setCollection(e.target.value)}>{collections.map(x=><option key={x} value={x}>{x==='all'?'All collections':x}</option>)}</select></div>
    <div className="picker-grid">{query === undefined ? <div className="skeleton" style={{height:180,gridColumn:'1/-1'}}/> : filtered.length ? filtered.map(item=><button className={`picker-item${selected.includes(item.url!) ? ' selected' : ''}`} key={item._id} onClick={()=>toggle(item.url!)} aria-pressed={selected.includes(item.url!)}><img src={item.url!} alt={item.alt}/><span>{item.filename}</span><small>{item.width && item.height ? `${item.width} × ${item.height}` : 'Image'}</small><span className="picker-check">{selected.includes(item.url!) ? '✓' : ''}</span><span className="picker-preview" onClick={e=>{e.stopPropagation();setPreview(item.url!)}}>Preview</span></button>) : <p className="picker-empty">No images match. Upload an image below.</p>}</div>
