...
export function ProjectsList() {
  ...
  const filtered = projects.filter(p => (status==='all'||p.status===status) && (featured==='all'||p.featured===(featured==='featured')) && `${p.title} ${p.description} ${p.tags.join(' ')}`.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>sort==='order'?a.order-b.order:sort==='title'?a.title.localeCompare(b.title):sort==='oldest'?a.updatedAt-b.updatedAt:b.updatedAt-a.updatedAt)
  ...
  return <div className="cms-list-page"><header className="cms-page-head"><div><h1>Projects</h1><p>Manage projects and showcase work.</p></div><div style={{display:'flex',gap:8}}><button disabled={projects.length < 2} onClick={()=>setArranging(true)}>Arrange projects</button><button onClick={()=>setEditing('new')}>+ New Project</button></div></header>
    ...
    {arranging && <ProjectOrderEditor projects={projects} onClose={()=>{setArranging(false);setSort('order')}} />}
